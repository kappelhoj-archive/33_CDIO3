...
public class GUIHandler {
	...
	private void showRoll(int[] currentDice) {
		...
		// GUI.setDice(currentDice[0],1+varians(),4+varians(),
		// currentDice[1],2+varians(),7+varians());
	}

	private String landOnInitial(String playerName, int playerPosition, int[] currentDice) {
	...
	public void landOnOwnable(String playerName, int playerPosition, int[] currentDice, String fieldType) {
	...
	public void landOnOwned(String playerName, int playerPosition, int[] currentDice, String fieldType,
			String fieldOwner, int rent) {
		String out = landOnInitial(playerName, playerPosition, currentDice);
		out += GameText.landOnFieldText(playerName, GameText.fieldTitles[playerPosition - 1]);
	...
	public void landOnRefuge(String playerName, int playerPosition, int amount, int[] currentDice) {
		String out = landOnInitial(playerName, playerPosition, currentDice);
		out += GameText.refugeText(playerName, GameText.fieldTitles[playerPosition - 1], amount);
	...
	public boolean landOnTax(String playerName, int playerPosition, int tax, int playerFortune, int[] currentDice) {
		...
		return GUI.getUserLeftButtonPressed(out, GameText.getButtonText("10%") +" " + playerFortune * 0.1, tax + "");
	}
	...
	public void boughtField(String playerName, int playerPosition, int playerBalance) {
		...
		GUI.getUserButtonPressed(GameText.boughtFieldText(playerName, GameText.fieldTitles[playerPosition - 1]),
				GameText.getButtonText("ok"));
	}
